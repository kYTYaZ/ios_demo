...
// for CVPixelBufferRef support
// If one callback implemented, you should implement all of them
/**
 *  Create a CVPixelBufferRef object. This object will be used by AgoraRtcEngineKit.
 *  SDK will render a video stream onto this object, then send notification
 *  using renderPixelBuffer
 *
 *  @param engine  The engine kit
 ...
 */
- (CVPixelBufferRef)rtcEngine:(AgoraRtcEngineKit *)engine createInputBufferWithWidth:(int)width height:(int)height stride:(int)stride;

/**
 *  Notification to delegate that video content from uid has been
 *  uploaded to CVPixelBufferRef object
 *
 *  @param engine       The engine kit
 *  @param uid          User ID (0 for local video, non 0 for remote user)
 *  @param pixelBuffer  CVPixelBufferRef object containing newly updated RGBA video
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine renderPixelBuffer:(NSString *)uid pixelBuffer:(CVPixelBufferRef)pixelBuffer;

/**
 *  Callback after local audio processed
 *
 *  @param engine       The engine kit
 *  @param audioData    Audio samples
 *  @param sampleRate   Sample rate
 *  @param numOfChannels Channels of audio
 *  @param bitDepth     Bit depth of audio sample
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine audioRecord:(NSData *)audioData sampleRate:(int)sampleRate numOfChannels:(int)numOfChannels bitDepth:(int)bitDepth;
@end
...
/**
 *  Enable/disable CVPixelBufferRef video frame observer
 */
- (int) enableVideoObserver;
- (int) disableVideoObserver;
/*
 * Enable/disable audio frame observer
*/
//- (int) enableAudioObserver;
//- (int) disableAudioObserver;
